...
export async function isWorkspaceLocked(config: TerraformConfig, workspace: TerraformWorkspace): Promise<boolean> {
  const workspaceInfo = await getWorkspace(config, workspace.id);
  return workspaceInfo?.data.attributes.locked ?? false;
}
...
export async function waitForRun(config: TerraformConfig, { id, status, workspace }: TerraformRun): Promise<void> {
  for (;;) {
...
async function getRun({ token }: TerraformConfig, id: string): Promise<RunResponse | null> {
  try {
    const client = getClient(token);
